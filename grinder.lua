...
grinder_recipes["default:obsidian"]		= {"default:obsidian_shard",	10}
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if fuel:get_count() < grind_consume_fuel or minetest.get_item_group(fuel:get_name(), "wood") == 0 then
			set_infotext(meta, 1)
			return
		end
...
if minetest.get_modpath("wrench") and wrench then
	...
	wrench:register_node("mining_plus:grinder", {
		lists = {"src", "dst", "fuel", "ej"},
		metas = {
			owner = STRING,
			infotext = STRING,
			formspec = STRING,
